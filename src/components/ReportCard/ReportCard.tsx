  ...
  return (
    ...
    >
      ...
      <div className="min-h-[76px] px-4 py-3 flex flex-col gap-1 justify-center">
        <h2 className="heading-m truncate w-full text-neutral-900">{title || 'Untitled Report'}</h2>
      ...
      {progressTags.length > 0 && (
        <div className="h-auto px-4 py-[10px] flex flex-wrap items-center gap-[10px] w-full">
      ...
      <div className="w-full min-h-auto p-[15px] flex flex-col gap-3 bg-white">
        ...
        {tags.length > 0 && (
          <div className="h-auto px-0 py-[10px] flex flex-wrap items-center gap-[10px] w-full">
            {tags.map((tag) => (
              <StatusPill key={tag} label={tag} />
            ))}
          </div>
        ...
        <div className="w-full">
          <Link to={isDraft ? `/create-post?id=${id}` : `/report/${id}`} className='link-m'>
            {isDraft ? 'Edit draft' : 'View report details'}
          </Link>
        </div>
      </div>
      <div className="w-full mx-auto h-[52px] px-4  flex justify-between items-start ">
        <p className='body-s text-neutral-900'>
          {isDraft ? 'Last saved ' : 'Posted '}{timestamp}
        </p>
        <p className='body-s text-neutral-900'>
          {postedBy}
        </p>
