...
const StepMediaContent = () => {
  ...
  const { openCamera } = useModalStore();

  const fileInputRef = useRef<HTMLInputElement>(null);
  const mediaList: EditableMedia[] = values.media || [];

  const { handleCapture, processFiles } = useMediaManager((item) => {
  ...
  const handleOpenCamera = () => {
    openCamera({
      onCapture: (fileData: { type: 'image' | 'video'; dataUrl: string; file?: File }) => {
  ...
  return (
    ...
    {editingMedia && (
      <EditMediaModal
        ...
        onDiscard={(id) => setFieldValue('media', mediaList.filter(item => item.id !== id))}
      />
    )}
  </>
);
